...
- **Components live in `packages/ui`**: All shadcn components are installed to `packages/ui/src/components/`
- **Import from `@bob/ui`**: Use `import { Button, Card, ... } from '@bob/ui'`
...
All components should match shadcn's visual language. Use shadcn components from `@bob/ui` whenever possible.

### Buttons

Use the `Button` component from `@bob/ui`. Follow variant semantics:
...
Use shadcn's `Card`, `CardHeader`, `CardTitle`, `CardDescription`, `CardContent` from `@bob/ui`:

```typescript
import { Card, CardHeader, CardTitle, CardDescription, CardContent } from '@bob/ui'
