...
description: This skill should be used when the user asks to "create a component", "build a React component", "add component props", "extend HTML attributes", "component interface", "layout structure", "component architecture", "props pattern", "slot props", "render props", "add a shadcn component", "install UI component", "use shadcn", or needs guidance on React component patterns, prop interfaces, and shadcn UI component usage in apps/app.
---

# React Components

This skill provides conventions for creating React components in the Worksauce frontend application (`apps/app`).

## When to Apply

- Creating new React components in `apps/app/app/components/`
...
| Location | Purpose |
|----------|---------|
| `apps/app/app/components/` | App-specific feature components |
| `packages/ui/src/components/` | Shared reusable UI components (shadcn) |
| `apps/app/app/routes/` | Route components (React Router v7) |

## shadcn UI Components

This project uses [shadcn/ui](https://ui.shadcn.com) for consistent, accessible UI components. All shadcn components are installed into `packages/ui/src/components/`.
...
```bash
# Install to packages/ui (run from packages/ui directory)
cd packages/ui && npx shadcn@latest add button
```

### Importing UI Components

All shadcn components are exported from `@bob/ui`:

```typescript
import { Button, Card, Input, Dialog } from '@bob/ui'
```
...
- Import types from `@bob/schema` when available
- Check shadcn registry before creating new UI components
- Install shadcn components to `packages/ui`
- Import UI components from `@bob/ui`
...
- Don't install shadcn components anywhere except `packages/ui`
...
- **`tanstack-query-hooks`** - Data fetching patterns with React Query
- **`ui-forms`** - Form handling with react-hook-form
